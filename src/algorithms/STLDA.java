...
public class STLDA extends TopKRecommender{
	...
	//public  HashMap<Integer,Location> user_home_location;
	  ...
	  //  public double alpha_sum;
	    ...
	    */
		...
		public STLDA(SparseMatrix trainMatrix, ArrayList<Rating> testRatings, 
	    ...
	    public STLDA(String check_in_file,int K,int R,  double alpha, double beta, double eta,
				double gamma, int U,int V,int W,String path_model_output,int ITERATIONS,int SAMPLE_LAG,int BURN_IN)
	    {
	    	...
	    	//topic_time_parameters=new double[K][2];
	    	//topic_time_parameters_sum=new double[K][2];
	    	...
	    	usercheckinset=new  HashMap<Integer,ArrayList<CheckInRecord>>();
	    	//ReadInput.load_checkin_data(check_in_file,usercheckinset);
	    	data_reshape();
	    	
	    	user_home_location=new Location[U];
	    	//poi_location=new Location[V];
	    	poi_region_innitilization=new int[V];
	    	...
	    	//pertopic_assignments_list=new HashMap<Integer,ArrayList<Double>>();
	    	
//	    	for(int i=0;i<K;i++)
//	    	{
//	    		ArrayList<Double> li=new ArrayList<Double>();
//	    		pertopic_assignments_list.put(i,li);
//	    	}
	    	
	    	System.out.println("STLDA model has been installed!");
	    }
	    
	    public void train(String KMeansFile)
	    {
	    ...
	    public void buildcrosscitymodel(int totaliter) {
	    ...
	    public void readcatefile(String catefile) throws IOException{
	    	readcategary_location(catefile);
	    };
	    ...
	    public void data_reshape() {
	    	...
	    	for (int u = 0;u<userCount;u ++) 
	    		for (int i = 0;i<itemCount;i++) {
	    			...
	    			if (!POIset.containsKey(i)) {
	    				int cate = poicate[i];
	    				ArrayList<Integer> list=new ArrayList<Integer>();
	    				for (int j = 0;j<3;j++)
	    					list.add(poiwords[i][j]);
	    				 POI p=new POI();
	    				 p.poi_id=i;
	    				 p.location=poi_location_topk[i];
	    				 p.wordset=list;
	    				 POIset.put(i, p);
	    			}
	    ...
	    public void crosscity_initialize(HashMap<Integer,ArrayList<Location>> perregion_assignments_list,int[] poi_region_inni){
          ...
          public void initializeModel(String KMeansFile)
          {
        	...
        	// initialize region assignments r
        	  ...
        	  crosscity_initialize(perregion_assignments_list,poi_region_innitilization);
