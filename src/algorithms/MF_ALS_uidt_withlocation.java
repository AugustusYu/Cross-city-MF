...
public class MF_ALS_uidt_withlocation extends TopKRecommender {
	/** Model priors to set. */
	...
	public MF_ALS_uidt_withlocation(SparseMatrix trainMatrix, ArrayList<Rating> testRatings, 
			...
			String catefile) throws IOException {
...
//		  double a = 0;
//		  Location b = poi_location_topk[i];
//		  double A = rad(l.latitude) - rad(b.latitude);
//		  double B = rad(l.longitude) - rad(b.longitude);
//		  double s = 2 * Math.asin(Math.sqrt(Math.pow(Math.sin(A / 2), 2) + 
//				  Math.cos(rad(l.latitude)) * Math.cos(rad(b.latitude)) * Math.pow(Math.sin(B / 2), 2)));
//		  s = s * 6378;
//		  if(s>location_threhold)
//			  a = -1;
//		  else
//			  a = Ufinal.row(user_index[u], false).inner(Vfinal.row(poi_index[i],false));
//		  return a ;
//	  }
	    ...
	    public void refresh_stlda_apras(String kmeansfile,int R) throws IOException{
	    	...
	    	poi_region_innitilization=new int[itemCount];
	    ...
	    private void read_kmeans_file( String kmeansfile) throws IOException{
			...
			while((line = reader.readLine()) != null) {
				...
				perregion_assignments_list.get(region).add(poi_location_topk[poi]);
				count ++;
			}
			reader.close();
	  ...
	  public double predict_currentlocal(int u ,int i ,Location l) {
		  ...
		  a = Ufinal.row(user_index[u], false).inner(Vfinal.row(poi_index[i],false))*GaussianProbability1(region,l);
		  return a ;
	  }
