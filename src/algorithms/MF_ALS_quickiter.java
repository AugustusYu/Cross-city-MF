 ...
 * @author xiangnanhe
 */
public class MF_ALS_quickiter extends TopKRecommender {
	/** Model priors to set. */
  ...
  /** Caches */
	...
	public  HashSet<Integer> nativeUsers ;
	public  HashSet<Integer> tourUsers ;
	public  HashSet<Integer> nativePois;
	public  HashSet<Integer> tourPois ;
	
	public MF_ALS_quickiter(SparseMatrix trainMatrix, ArrayList<Rating> testRatings, 
	...
	public void buildModel() {
		...
		for (int iter = 0; iter < maxIter; iter ++) {
			...
			if (showProgress && (iter > showbound || iter % showcount == 0)) {
				...
				evaluatefor82(testRatings,start);
			}
	...
	public void buildcrosscityModel() {
		...
		for (int iter = 0; iter < maxIter; iter ++) {
			...
			if (showProgress && (iter > showbound || iter % showcount == 0)) {
				long end_iter = System.currentTimeMillis();
				System.out.printf("iter = %d [%s]  ",iter,Printer.printTime(end_iter - start));
				evaluatefor82crosscity(testRatings,start,nativeUsers,tourPois);
			}
			//showProgress(iter, start, testRatings);			
			// Show loss
			if (showLoss)
